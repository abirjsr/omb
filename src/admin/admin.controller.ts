...
@Post('departments/by-email')
@UseGuards(SessionGuard)
async createDepartment(
  @Body() createDto: CreateDepartmentDto
) {
  return this.adminService.createDepartment(createDto.adminEmail, createDto);
}

// Or if you want to keep the email in the path
@Post('departments/by-email/:adminEmail')
@UseGuards(SessionGuard)
async createDepartmentByEmail(
  @Param('adminEmail') adminEmail: string,
  @Body() createDto: CreateDepartmentDto
) {
  return this.adminService.createDepartment(adminEmail, createDto);
}
...
@Get('departments/by-email/:adminEmail')
@UseGuards(SessionGuard)
async getAdminDepartmentsByEmail(
  @Param('adminEmail') adminEmail: string
): Promise<Department[]> {
  return this.adminService.getAdminDepartmentsByEmail(adminEmail);
}

@Put('departments/by-email/:adminEmail/:employeeId')
@UseGuards(SessionGuard)
async updateDepartmentByAdminEmail(
  ...
  return this.adminService.updateDepartmentByAdminEmail(adminEmail, employeeId, updateDto);
}

@Delete('departments/by-email/:adminEmail')
@UseGuards(SessionGuard)
async deleteDepartmentByEmail(
  @Param('adminEmail') adminEmail: string
  
): Promise<{ message: string }> {
  await this.adminService.deleteDepartmentByEmail(adminEmail);
...
@Get('employee-tasks/task/:taskId')
 // @UseGuards(SessionGuard)
  async getEmployeeTaskById(
    @Param('taskId') taskId: string
  ): Promise<EmployeeTask> {
    return this.adminService.getEmployeeTaskById(taskId);
  }
...
// Add this to your employee-task.controller.ts
//  @Delete('employee-tasks/:taskId')
// @UseGuards(SessionGuard)
// async deleteEmployeeTask(
//   @Param('taskId') taskId: string
// ): Promise<void> {
//   return this.adminService.deleteEmployeeTask(taskId);
// }
...
@UseGuards(SessionGuard)
async deleteEmployeeTask(
  @Param('taskId', ParseUUIDPipe) taskId: string // Add ParseUUIDPipe here
): Promise<void> {
  return this.adminService.deleteEmployeeTask(taskId);
}
